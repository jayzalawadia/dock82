...
app.patch('/api/admin/users/:id', async (req, res) => {
  try {
    ...
    if (homeownerStatus !== undefined) {
      ...
      const existingPermissions = parsePermissions(existingUser.permissions);
      if (isHomeowner) {
        const verifiedAt =
          sanitizedStatus && ['verified', 'active_owner', 'active'].includes(sanitizedStatus)
            ? new Date().toISOString()
            : null;
        updatePayload.permissions = applyHomeownerStatusToPermissions(
          existingPermissions,
          sanitizedStatus || null,
          verifiedAt
        );
      } else {
        updatePayload.permissions = applyHomeownerStatusToPermissions(existingPermissions, null);
      }
    }
